...
from train_model import load_data_from_db, prepare_data, safe_divide
...
class TestLoadDataFromDb:
    ...
    def test_load_data_from_db_mock(self):
        ...
        mock_sirh = pd.DataFrame({"id_employee": [1, 2, 3], "age": [30, 35, 40], "revenu_mensuel": [5000, 6000, 7000]})

        mock_eval = pd.DataFrame({"eval_number": ["E_1", "E_2", "E_3"], "note_evaluation": [4.0, 4.5, 3.8]})

        mock_sondage = pd.DataFrame({"code_sondage": [1, 2, 3], "satisfaction": [4, 5, 3]})
